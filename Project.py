...
while True:
    ...
    for hand in result.hand_landmarks:
        for point1_idx, point2_idx, point3_idx in finger_angle_points:
            ...
            frs_finger = calculate_angle(hand[1],hand[2],hand[3])
            sec_finger = calculate_angle(hand[5], hand[6], hand[7])
            thr_finger = calculate_angle(hand[9], hand[10], hand[11])
            for_finger = calculate_angle(hand[13], hand[14], hand[15])
            fi_finger = calculate_angle(hand[17], hand[18], hand[19])
            if frs_finger >= angle_threshold and sec_finger>=angle_threshold and thr_finger < angle_threshold and for_finger < angle_threshold and fi_finger < angle_threshold:
                print("OKAY")
    ...
    results = yolo.predict(
        ...
        conf=0.015,      # Confidence Threshold
    ...
    # 신호등 Crop
    selected_box = min(results.boxes, key=lambda box:float(box.xyxy[0][1].item()))
